...
The system SHALL display an expandable hierarchical list of sites with their associated floor plans, similar to the View tab pattern.

#### Scenario: Display sites with floor plans
- **WHEN** user opens Floor Plan tab
- **THEN** system displays all sites in expandable disclosure groups
- **AND** each site shows count and list of floor plans when expanded
- **AND** sites are sorted alphabetically

#### Scenario: Empty site with no floor plans
...
The following requirements from the original specification were not implemented in the current version:

### ~~Requirement: Camera Information Panel~~
**Reason**: The implementation does not use a separate slide-up information panel. Instead, device information is shown directly in the streaming header, and live streaming starts immediately upon device selection.

### ~~Requirement: Camera Live Streaming with Long-Press Gesture~~
**Reason**: Live streaming now activates with a single tap (device selection), not a long-press gesture. The long-press interaction pattern was simplified to single-tap for better usability.

### ~~Requirement: Nested Modal for Live Streaming~~
**Reason**: Streaming is integrated inline using split-screen design with the floor plan, not presented as a nested modal using MultipleView component. This provides better spatial context.

### ~~Requirement: Offline Behavior with Timestamps~~
**Reason**: Offline caching and "Last updated" timestamp indicators are not implemented in the current version. This may be added in a future iteration if needed.
