...
The floor plan ViewModels SHALL use dependency injection for FloorPlanManager instead of direct VortexRestfulApi access to maintain proper architectural layering.

**Previous behavior (mobile-floor-plan-code-quality spec):** ViewModels used `@Dependency(\.vortexRestfulApi)` to make direct API calls.

**New behavior:** ViewModels use `@Dependency(\.floorPlanManager)` to delegate data operations to Manager layer.

#### Scenario: ViewModels use injected FloorPlanManager dependency
- **WHEN** FloorPlanTabViewModel needs to fetch floor plan data
- **THEN** ViewModel uses `@Dependency(\.floorPlanManager)` injection
- **AND** ViewModel calls Manager methods on injected dependency
- **AND** ViewModel does NOT use `@Dependency(\.vortexRestfulApi)`

#### Scenario: FloorPlanManager is restored as dependency
- **WHEN** floor plan code is refactored
- **THEN** FloorPlanManager.swift file exists in VortexFeatures/Core/FloorPlanManager/
- **AND** FloorPlanManagerProtocol.swift file exists
- **AND** FloorPlanManager is registered as dependency
- **AND** Manager makes API calls internally via VortexRestfulApi

#### Scenario: Manager calls in ViewModels are testable
- **WHEN** ViewModels use FloorPlanManager dependency injection
- **THEN** tests can inject MockFloorPlanManager
- **AND** Manager behavior can be mocked and verified in tests
- **AND** no API calls are made during ViewModel unit tests
...
- **AND** ViewModel calls `deviceManager.allSites()` for site list
- **AND** ViewModel passes sites to FloorPlanManager when needed

### Requirement: ViewModel Unit Test Coverage with Manager Mocks
The floor plan ViewModel tests SHALL use MockFloorPlanManager instead of MockVortexRestfulApi to properly test UI layer independently from data layer.

**Previous behavior (mobile-floor-plan-code-quality spec):** ViewModel tests used MockVortexRestfulApi to mock API responses directly.

**New behavior:** ViewModel tests use MockFloorPlanManager to mock Manager responses, focusing on UI logic only.

#### Scenario: FloorPlanTabViewModel tests use MockFloorPlanManager
- **WHEN** FloorPlanTabViewModel is tested
- **THEN** tests inject MockFloorPlanManager dependency
- **AND** mock provides UI models (FloorPlanItem) not Backend models
- **AND** tests verify UI state changes (isLoading, floorPlans array updates)
- **AND** tests verify user interactions (tapFloorPlan, pullToRefresh)
- **AND** tests do NOT verify API calls or data transformation

#### Scenario: FloorPlanDetailViewModel tests use MockFloorPlanManager
- **WHEN** FloorPlanDetailViewModel is tested
- **THEN** tests inject MockFloorPlanManager dependency
- **AND** mock provides device positions as UI models
- **AND** tests verify device selection state changes
- **AND** tests verify zoom/pan state management
- **AND** tests do NOT verify API integration

#### Scenario: ViewModel tests remain at 80%+ coverage
- **WHEN** ViewModels are refactored to use Manager
- **THEN** all 39 existing ViewModel tests still pass
- **AND** tests still achieve 80%+ code coverage
- **AND** tests focus on UI logic (state changes, user interactions)
- **AND** data logic testing moved to Manager tests

### Requirement: Floor Plan Data Management through Manager Layer
The system SHALL manage floor plan data through dependency-injected FloorPlanManager that wraps VortexRestfulApi access.

**Previous behavior (mobile-floor-plan-code-quality spec):** ViewModels directly called VortexRestfulApi methods for data fetching.

**New behavior:** ViewModels call FloorPlanManager methods; Manager internally calls VortexRestfulApi.

#### Scenario: Fetch floor plans through Manager
- **WHEN** user opens Floor Plan tab
- **THEN** FloorPlanTabViewModel calls `floorPlanManager.fetchAllFloorPlans(sites:)`
- **AND** Manager internally calls `vortexRestfulApi.getFloorPlans(siteID:)`
- **AND** Manager transforms Backend models to UI models
- **AND** Manager adds deviceCount by fetching device positions
- **AND** ViewModel receives [FloorPlanItem] ready for display

#### Scenario: Fetch device positions through Manager
- **WHEN** user opens floor plan detail view
- **THEN** FloorPlanDetailViewModel calls `floorPlanManager.fetchDevicePositions(forFloorPlanID:)`
- **AND** Manager internally calls `vortexRestfulApi.getDevicePositions(floorPlanID:)`
- **AND** Manager transforms Backend models to UI models
- **AND** ViewModel receives [DevicePosition] ready for display

#### Scenario: Handle API failure in Manager layer
- **WHEN** floor plan API request fails
- **THEN** Manager catches API error
- **AND** Manager logs error via VortexLogger
- **AND** Manager throws descriptive error to ViewModel
- **AND** ViewModel updates UI state (isLoading = false)
- **AND** ViewModel can display error to user
