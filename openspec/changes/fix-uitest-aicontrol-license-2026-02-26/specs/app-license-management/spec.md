...
### Requirement: License phase UITest tab validation
The `LicensePhaseUITest.checkTabItemStatus()` SHALL validate the correct set of top-level tabs based on the current home view tab structure.

#### Scenario: Tab check reflects AI Hub integration
- **WHEN** `checkTabItemStatus(_ isEnable: Bool)` is executed during a License phase test
- **THEN** the function SHALL check the following tabs: `.view` (always enabled), `.customizedView`, `.aiHub`, `.message`
- **AND** SHALL NOT check `.deepSearch` (no longer a top-level tab since DeepSearch integration into AI Hub)

#### Scenario: AI Hub tab state matches license phase
- **GIVEN** the device is in grace period license phase
- **WHEN** `checkTabItemStatus(true)` is called
- **THEN** `.aiHub` tab SHALL be verified as enabled

#### Scenario: AI Hub tab disabled in overdue license phase
- **GIVEN** the device is in overdue license phase
- **WHEN** `checkTabItemStatus(false)` is called
- **THEN** `.aiHub` tab SHALL be verified as disabled
