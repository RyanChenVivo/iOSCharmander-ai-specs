...
**IMPORTANT**: Credentials are stored in a `.env` file in the project root directory.

### Step 1: Check for credentials

Run: `./scripts/check_credentials.sh`
...
```
I've created a .env file in the project root and added it to .gitignore. Please edit .env and fill in your Confluence credentials:
...
```
The .env file exists but credentials are not properly configured. Please edit .env and ensure these values are set:
...
Run: `./scripts/verify_credentials.sh`
...
3. **For /display/ URLs, lookup page ID first**:
   ...
   Run: `./scripts/lookup_page_by_title.sh <space_key> "<title>"`

   Example: `./scripts/lookup_page_by_title.sh PP2 "ACaaS - Brivo Event Type List"`

   Parse the JSON response to get the page ID from `results[0].id`

4. **Fetch and convert page**:

   Run: `source .env && ./scripts/fetch_page.sh <page_id> ./confluence-pages`

   This script:
   - Fetches page via REST API
   - Saves raw JSON to `confluence-pages/raw/page_{pageId}_raw.json`
   - Converts to Markdown using `confluence_to_markdown.py`
   - Saves to `confluence-pages/{Page_Title}.md`

5. **Download images**:

   Run: `./scripts/download_images.sh <page_id> confluence-pages/raw/page_<page_id>_raw.json ./confluence-pages`
...
6. **Update attachment references**:
   ...
   import json, re
   d = json.load(open('confluence-pages/raw/page_<page_id>_raw.json'))
   ...
   Then run: `python3 ./scripts/update_attachment_refs.py <markdown_file> <page_id> "$TITLE_PREFIX"`
