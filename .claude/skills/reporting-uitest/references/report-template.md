...
| 失敗群組 | 優先級 | 業務影響 | 建議處理 | 具體行動 | 狀態 |
|----------|--------|----------|----------|----------|------|
| {{group_name}} | {{priority}} | {{business_impact}} | {{recommendation}} | {{specific_action}} | {{status}} |

**狀態說明:**
- ✅ 已完成 - 已執行建議行動
- ⏳ 待處理 - 尚未處理，下次 session 可繼續
- 🔄 進行中 - 已開始但未完成（如已建立 proposal 但未實作）
